// Pages Component - Analytics Dashboard
// GA4-style Custom Filters Implementation
...
const getFinalEnglishTitle = (title, page) => {
  ...
  useEffect(() => {
    ...
    if (urlPage && urlPage !== activeTab) {
      setActiveTab(urlPage)
    }
    
    // Set default URL params if not present
    if (!urlPeriod && !urlPage) {
      setSearchParams({ period: '7', page: 'entry' })
    }
  }, [searchParams])
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      // Read current period from URL parameters
      const currentPeriod = searchParams.get('period') || '7'
      console.log('Pages - Loading initial data with period:', currentPeriod)
      ...
      console.log('🔍 Pages - Filter context state:', filters)
      console.log('🔍 Pages - Using filters:', filterParams)
      console.log('🔍 Pages - Filter params count:', Object.keys(filterParams).length)

      

      // Load first chunk (10 items each) with offset 0 and filters

      const [mostVisitedRes, entryPagesRes, exitPagesRes] = await Promise.all([

        pagesAPI.getMostVisited(projectId, 10, startDate, endDate, 0, filterParams),

        pagesAPI.getEntryPages(projectId, 10, startDate, endDate, 0, filterParams),

        pagesAPI.getExitPages(projectId, 10, startDate, endDate, 0, filterParams)
      ...
      console.log('📊 Pages Initial Load - FILTERED DATA:')

      console.log('  Most Visited:', most_visited.length, 'items')

      console.log('  Entry Pages:', entry_pages.length, 'items')

      console.log('  Exit Pages:', exit_pages.length, 'items')

      console.log('  Most Visited Sample:', most_visited.slice(0, 2))

      console.log('  Entry Pages Sample:', entry_pages.slice(0, 2))



      // Clear existing data before setting filtered data
      setMostVisited([])
      setEntryPages([])
      setExitPages([])

      // Force re-render with new filtered data
      setMostVisited([...most_visited])
      setEntryPages([...entry_pages])
      setExitPages([...exit_pages])

      console.log('✅ State updated with filtered data:', {
        mostVisited: most_visited.length,
        entryPages: entry_pages.length,
        exitPages: exit_pages.length
      })
  ...
  const loadMoreData = async () => {
    ...
    try {
      ...
      if (activeTab === 'entry') {
        ...
        response = await pagesAPI.getEntryPages(projectId, 10, startDate, endDate, currentOffset, filterParams)

      } else if (activeTab === 'top') {

        response = await pagesAPI.getMostVisited(projectId, 10, startDate, endDate, currentOffset, filterParams)

      } else {

        response = await pagesAPI.getExitPages(projectId, 10, startDate, endDate, currentOffset, filterParams)
  ...
  const handlePeriodChange = (newPeriod) => {
    ...
    // Update URL parameters instead of localStorage
    setSearchParams({
      period: newPeriod,
      page: activeTab
    })
      ...
      <div className="content">
        ...
        <div style={{
          ...
          <div
            ...
            onClick={() => {
              setActiveTab('entry')
              setSearchParams({ period: period, page: 'entry' })
            }}
          ...
          <div
            ...
            onClick={() => {
              setActiveTab('top')
              setSearchParams({ period: period, page: 'top' })
            }}
          ...
          <div
            ...
            onClick={() => {
              setActiveTab('exit')
              setSearchParams({ period: period, page: 'exit' })
            }}
        ...
        <div className="chart-container" style={{ padding: 0, maxHeight: 'none', overflow: 'hidden' }} key={filters.length}>
          ...
          <div style={{ padding: '10px' }}>
            ...
            {getCurrentData().length > 0 ? (
              <div>
                ...
                {console.log(`🔍 Rendering ${getCurrentData().length} pages in ${activeTab} tab - Filters: ${filters.length} active`)}

                {getCurrentData().map((page, idx) => (

                  <div

                    key={`${idx}-${filters.length}`}

                    style={{

                      borderBottom: idx < getCurrentData().length - 1 ? '1px solid #e2e8f0' : 'none',
