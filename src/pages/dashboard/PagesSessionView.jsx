...
// Translation function for any language to English
const translateToEnglish = (text) => {
  if (!text) return text
  
  // Check if text contains non-English characters (Arabic, Hindi, etc.)
  const hasNonEnglishChars = /[\u0600-\u06FF\u0900-\u097F]/.test(text)
  
  if (!hasNonEnglishChars) {
    return text // Return as-is if it's already English
  }
  
  // Common translations for dashboard content
  const translations = {
    // Arabic translations
    'ملابس قطنية مطبوعة يدوياً وصلت حديثاً - متجر هائي - الصفحة 9': 'Hand-printed cotton clothes newly arrived - Hathi Store - Page 9',
    'متجر هائي': 'Hathi Store',
    'ملابس قطنية': 'Cotton Clothes',
    'مطبوعة يدوياً': 'Hand Printed',
    'وصلت حديثاً': 'Newly Arrived',
    'الصفحة': 'Page',
    
    // Hindi translations
    'हाथी स्टोर': 'Hathi Store',
    'सूती कपड़े': 'Cotton Clothes',
    'हाथ से छपे': 'Hand Printed',
    'नए आए': 'Newly Arrived',
  ...
  Object.keys(translations).forEach(arabicWord => {
    if (translatedText.includes(arabicWord)) {
      translatedText = translatedText.replace(new RegExp(arabicWord, 'g'), translations[arabicWord])
    }
  })
  
  // If still contains non-English characters, provide a generic English fallback
  if (/[\u0600-\u06FF\u0900-\u097F]/.test(translatedText)) {
    // Extract numbers and basic structure, replace text with English equivalent
    translatedText = translatedText
      .replace(/[\u0600-\u06FF\u0900-\u097F]+/g, 'Translated Page')
      .replace(/(\d+)/g, 'Page $1')
  }
...
function PagesSessionView({ projectId, selectedPageSessions, pageType, onBack, project }) {
  ...
  const getDateRange = (days) => {
    // Get current date in IST - Fixed to use actual current date
    const now = new Date()
    ...
    // For start date, go back by (days - 1) days to include today
    // Example: 30 days = today + last 29 days = 30 total days
  ...
  return (
    <>
      <div className="header" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '4px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '98%' }}>
          <div>
            <h1 style={{ margin: 0 }}>{translateToEnglish(pageType === 'entry' ? 'Entry Page' : pageType === 'top' ? 'Top Page' : 'Exit Page') + ' Sessions'}</h1>
            <div style={{ fontSize: '14px', color: '#64748b', marginTop: '4px', marginBottom: '12px' }}>
              {translateToEnglish(selectedPageSessions?.title) || selectedPageSessions?.page || selectedPageSessions?.url}
            </div>
            <button
              onClick={onBack}
              style={{
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{
                            ...
                            {translateToEnglish(page.title) || 'Untitled Page'}
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
                            <a
                              href={page.url}
